...
-- Modificar a função get_empresa_dashboard_metrics para usar intervalo de datas
DROP FUNCTION IF EXISTS get_empresa_dashboard_metrics(uuid);

CREATE OR REPLACE FUNCTION get_empresa_dashboard_metrics(
    p_empresa_id uuid,
    p_start_date date,
    p_end_date date
)
...
BEGIN
    ...
    -- Evolução mensal com intervalo de datas personalizado
    SELECT json_agg(
        json_build_object(
    ...
    -- Distribuição por cargos (filtrada pelo intervalo de datas)
    SELECT json_agg(
        json_build_object(
            'cargo', cargo,
            'count', count
...
-- Conceder permissões
GRANT EXECUTE ON FUNCTION get_empresa_dashboard_metrics(uuid, date, date) TO authenticated;
