...
BEGIN
    ...
    -- CORREÇÃO DEFINITIVA: Evolução mês a mês (não cumulativa)
    WITH six_months AS (
        -- 1. Gera a série dos últimos 6 meses
    ...
    monthly_hires AS (
        -- 2. Calcula quantos funcionários novos entraram em cada mês
        SELECT
    ...
    monthly_costs AS (
        -- 3. Calcula o custo total dos planos iniciados em cada mês
        SELECT
    ...
    -- 4. Junta tudo, garantindo que todos os meses apareçam, mesmo que com valor zero
    SELECT json_agg(
        json_build_object(
