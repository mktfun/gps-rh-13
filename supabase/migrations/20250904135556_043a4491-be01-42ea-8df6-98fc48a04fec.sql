...
BEGIN
    ...
    -- CORREÇÃO: Calcular custo mensal total = soma(valor_mensal * funcionários_ativos por CNPJ)
    SELECT COALESCE(SUM(dp.valor_mensal * fa.funcionarios_ativos), 0) INTO custo_mensal_total
    FROM dados_planos dp
    INNER JOIN cnpjs c ON dp.cnpj_id = c.id
    LEFT JOIN (
        SELECT cnpj_id, COUNT(*) as funcionarios_ativos
        FROM funcionarios
        WHERE status = 'ativo'
        GROUP BY cnpj_id
    ...
    -- CORREÇÃO: Buscar custos por CNPJ com cálculo correto
    SELECT json_agg(
        json_build_object(
            'cnpj', c.cnpj,
    ...
    WHERE c.empresa_id = p_empresa_id
    AND c.status = 'ativo'
    AND (dp.id IS NOT NULL OR fa.funcionarios_count > 0);

    -- CORREÇÃO: Evolução mensal - total de funcionários ativos por mês (platô)
    SELECT json_agg(
        json_build_object(
