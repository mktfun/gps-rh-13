...
-- Corrigir a função get_empresas_com_metricas para evitar registros nulos
...
BEGIN
  ...
  -- CORREÇÃO CRÍTICA: Filtrar registros nulos e melhorar autorização
  WHERE e.id IS NOT NULL 
    AND e.nome IS NOT NULL 
    AND e.responsavel IS NOT NULL
    AND e.email IS NOT NULL
    AND (
      (e.corretora_id = auth.uid()) OR 
      EXISTS (
