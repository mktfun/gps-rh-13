...
-- Função para buscar relatório detalhado de funcionários da empresa
CREATE OR REPLACE FUNCTION get_funcionarios_report(
    p_empresa_id uuid,
...
BEGIN
    ...
    SELECT 
        COUNT(*) as total,
        COUNT(CASE WHEN f.status = 'ativo' THEN 1 END) as ativos,
        COUNT(CASE WHEN f.status != 'ativo' THEN 1 END) as inativos
    ...
    WITH evolucao_temporal AS (
        SELECT 
            to_char(date_trunc('month', generate_series(p_start_date, p_end_date, '1 month')), 'YYYY-MM') as mes,
            to_char(date_trunc('month', generate_series(p_start_date, p_end_date, '1 month')), 'Mon YYYY') as mes_nome
        FROM generate_series(p_start_date, p_end_date, '1 month') gs
    ),
    funcionarios_por_mes AS (
        SELECT 
            ...
            COUNT(CASE WHEN f.status != 'ativo' THEN 1 END) as funcionarios_inativos,
    ...
    distribuicao_status AS (
        SELECT 
            f.status,
            COUNT(*) as quantidade,
            ROUND((COUNT(*)::numeric / v_total_funcionarios::numeric) * 100, 2) as percentual
        FROM funcionarios f
        INNER JOIN cnpjs c ON f.cnpj_id = c.id
        WHERE c.empresa_id = p_empresa_id
        AND (p_cnpj_filter IS NULL OR c.id = p_cnpj_filter)
        ...
        'evolucao_temporal', COALESCE((
            SELECT jsonb_agg(
                jsonb_build_object(
                    ...
                    'funcionarios_ativos', COALESCE(fpm.funcionarios_ativos, 0),
                    'funcionarios_inativos', COALESCE(fpm.funcionarios_inativos, 0),
                    'novas_contratacoes', COALESCE(fpm.novas_contratacoes, 0),
                    'desligamentos', COALESCE(fpm.desligamentos, 0)
                )
            )
...
GRANT EXECUTE ON FUNCTION get_funcionarios_report(uuid, date, date, text, uuid, text) TO authenticated;
