-- Reverter função get_empresa_dashboard_metrics para versão original
-- Problema: estava aplicando cálculo automático em duplicidade para planos que já tinham valores
...
BEGIN
    ...
    -- Custo mensal total (incluir TODOS os tipos de planos, mas sem cálculo automático)
    SELECT COALESCE(SUM(dp.valor_mensal), 0) INTO custo_mensal_total
    FROM dados_planos dp
    JOIN cnpjs c ON dp.cnpj_id = c.id
    WHERE c.empresa_id = p_empresa_id
      AND c.status = 'ativo'
      AND dp.tipo_seguro IN ('vida', 'saude'); -- Incluir vida e saúde

    -- Custos por CNPJ (sem cálculo automático)
    SELECT json_agg(
      json_build_object(
        'cnpj', c.cnpj,
    ...
    FROM cnpjs c
    LEFT JOIN LATERAL (
      SELECT SUM(dp.valor_mensal) AS valor_total
      FROM dados_planos dp 
    ...
    -- Evolução mensal (incluir TODOS os tipos de planos)
    WITH meses AS (
    ...
    -- Plano principal (incluir todos os tipos, sem cálculo automático)
...
'Retorna métricas do dashboard da empresa incluindo TODOS os tipos de planos (vida + saúde) usando os valores configurados no banco, sem cálculo automático adicional.';
