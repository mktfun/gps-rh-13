-- Versão corrigida: Atualizar a função get_relatorio_custos_empresa para incluir planos de saúde
-- Removendo referência à coluna valor_mensal_calculado que não existe

-- First, drop the existing function
DROP FUNCTION IF EXISTS get_relatorio_custos_empresa(uuid,integer,integer);

-- Then, recreate the function with corrected implementation
...
DECLARE
  ...
  v_custo_medio_por_cnpj NUMERIC := 0;
  v_total_count BIGINT;
BEGIN
  -- Calcular totais globais incluindo tanto seguros de vida quanto planos de saúde
  WITH dados_agregados AS (
    SELECT 
      COUNT(DISTINCT CASE WHEN f.status = 'ativo' THEN f.id END)::BIGINT as funcionarios_ativos,
      COUNT(DISTINCT CASE WHEN dp.valor_mensal > 0 THEN c.id END)::BIGINT as cnpjs_com_plano,
      COALESCE(SUM(DISTINCT CASE WHEN dp.valor_mensal > 0 THEN dp.valor_mensal ELSE 0 END), 0)::NUMERIC as total_planos
    FROM cnpjs c
  ...
  WITH funcionarios_com_dados AS (
    SELECT DISTINCT
      ...
      ), 0)::NUMERIC as valor_plano_total,
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE 0
    END::NUMERIC as valor_individual,
    fcd.status::TEXT,
