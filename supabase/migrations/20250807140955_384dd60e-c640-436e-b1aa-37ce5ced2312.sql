...
-- Corrigir erro de agregações aninhadas na função get_detailed_costs_report
...
BEGIN
    ...
    monthly_data AS (
        SELECT 
            date_trunc('month', pb.created_at)::date AS month,
            SUM(pb.valor_mensal) AS total_cost
        FROM planos_base pb
        WHERE pb.created_at >= date_trunc('month', p_start_date::date)
        AND pb.created_at <= p_end_date
        GROUP BY date_trunc('month', pb.created_at)::date
    ),
    evolucao_temporal AS (
        ...
        LEFT JOIN monthly_data md ON month_series.month = md.month
    ),
    cnpj_aggregated AS (
        SELECT 
            rc.id,
