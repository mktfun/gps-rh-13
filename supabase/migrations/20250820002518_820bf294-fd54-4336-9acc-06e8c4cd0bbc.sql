...
WITH CHECK (
  auth.uid() = id AND 
  (OLD.role IS NOT DISTINCT FROM NEW.role OR get_my_role() = 'admin')
);
...
-- 6. Add security audit trigger for profile changes (FIXED)
...
AS $$
BEGIN
  -- Log role changes specifically (only for UPDATE operations)
