...
-- Primeiro, criar o ENUM para tipos de seguro
CREATE TYPE tipo_seguro AS ENUM ('vida', 'saude', 'outros');

-- Adicionar o campo tipo_seguro na tabela dados_planos
ALTER TABLE public.dados_planos 
ADD COLUMN tipo_seguro tipo_seguro DEFAULT 'vida';

-- Atualizar todos os registros existentes para 'vida' (assumindo que são seguros de vida)
UPDATE public.dados_planos 
SET tipo_seguro = 'vida' 
WHERE tipo_seguro IS NULL;

-- Tornar o campo obrigatório
ALTER TABLE public.dados_planos 
ALTER COLUMN tipo_seguro SET NOT NULL;

-- Criar índice para otimizar queries por tipo de seguro
CREATE INDEX idx_dados_planos_tipo_seguro ON public.dados_planos(tipo_seguro);

-- Atualizar a função get_pulse_financeiro_corretor com nova lógica de comissão
