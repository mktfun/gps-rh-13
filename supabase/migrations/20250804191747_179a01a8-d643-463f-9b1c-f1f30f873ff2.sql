...
-- Apague a função antiga antes para garantir que não haja conflitos
DROP FUNCTION IF EXISTS public.get_conversas_usuario();

-- Crie a nova função inteligente
...
BEGIN
  ...
  IF v_user_role = 'corretora' THEN
    ...
    RETURN QUERY
    SELECT c.id, e.nome, c.created_at
