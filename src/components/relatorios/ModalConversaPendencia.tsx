...
const sugestoes = [
  'Olá! Poderia enviar a documentação pendente para concluirmos o atendimento?',
  'Estamos acompanhando esta pendência. Há alguma atualização?',
  'Lembrete: o prazo desta pendência está próximo. Posso ajudar com algo?'
];

const ModalConversaPendencia: React.FC<Props> = ({ open, onOpenChange, pendencia }) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const iniciarOuObter = async () => {
      const { data, error } = await (supabase as any).rpc('iniciar_ou_obter_conversa_por_protocolo', {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        ...
        {!pendencia ? (
          <div className="text-center text-muted-foreground">Selecione uma pendência</div>
        ) : (
          <div className="space-y-4">
            ...
            <Card className="p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {sugestoes.map((s, i) => (
                  <Button key={i} variant="secondary" onClick={() => setNovaMensagem(s)}>
                    {s}
                  </Button>
                ))}
              </div>
