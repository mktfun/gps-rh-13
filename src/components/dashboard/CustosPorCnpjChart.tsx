...
const CustosPorCnpjChart = ({
  dados
}: CustosPorCnpjChartProps) => {
  ...
  const chartData = dados.filter(item => {
  ...
  if (!dados || dados.length === 0) {
    ...
    return <div className="flex flex-col items-center justify-center h-[300px] text-gray-500">
  ...
  return <div className="space-y-4">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          <Pie data={chartData} cx="50%" cy="50%" innerRadius={40} outerRadius={80} paddingAngle={2} dataKey="value" nameKey="name" stroke="#FFFFFF" strokeWidth={3}>
            {chartData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
          </Pie>
          <Tooltip content={<CustomTooltip />} />
          <Legend layout="vertical" verticalAlign="middle" align="right" formatter={value => <span className="text-sm">{value}</span>} />
        </PieChart>
      </ResponsiveContainer>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2 max-h-40 overflow-y-auto">
          ...
          return <div key={index} className="flex items-center justify-between p-2 bg-muted/50 rounded">
